...
public class Brick {
	...
	/**
	 * Create the brick at the given x and y coordinates, using specified images
	 * 
	 * @param x
	...
	public void initialiseBrickImgs() {
		...
		brickImgs[0] = new Sprite(new Texture(
				Gdx.files.classpath("imgs/green.png")));
		brickImgs[1] = new Sprite(new Texture(
				Gdx.files.classpath("imgs/red.png")));
		brickImgs[2] = new Sprite(new Texture(
				Gdx.files.classpath("imgs/yellow.png")));
		brickImgs[3] = new Sprite(new Texture(
				Gdx.files.classpath("imgs/blue.png")));
		brickImgs[4] = new Sprite(new Texture(
				Gdx.files.classpath("imgs/orange.png")));
		brickImgs[5] = new Sprite(new Texture(
				Gdx.files.classpath("imgs/purple.png")));
	}

	/**
	 * 
	 * @return the state of the brick i.e. whether the brick has been hit or not
	 */
	 ...
	 * 
	 * @param state
	 *            - sets the current state of the brick i.e. if the brick has
	 *            been hit set state to be false
	 */
	...
	/**
	 * Creates a small rectangle on the left of the brick to detect whether the
	 * ball hits the brick on this side, so we can handle the velocity
	 * accordingly
	 * 
	 * @param ball
	 *            - the current game ball
	 * @return true if the ball has intersected this side, false otherwise
	 */
	...
	/**
	 * Creates a small rectangle on the right of the brick to detect whether the
	 * ball hits the brick on this side, so we can handle the velocity
	 * accordingly
	 * 
	 * @param ball
	...
	/**
	 * Creates a small rectangle on the top of the brick to detect whether the
	 * ball hits the brick on this side, so we can handle the velocity
	 * accordingly
	 * 
	 * @param ball
	...
	/**
	 * Creates a small rectangle on the bottom of the brick to detect whether
	 * the ball hits the brick on this side, so we can handle the velocity
	 * accordingly
	 * 
	 * @param ball
	 *            - the current game ball
	 ...
	 * @param b
	 *            - contains the rendering information
	 * @param index
	 *            - relates to the bricks to be rendered in the first level
	 */
	 ...
	 * 
	 * @param num
	 *            - a number used to determine what bricks to assign specific
	 *            images
	 * @param b
	 *            - contains the rendering method
	 */
	 ...
	 *            - contains the rendering method
	 * @param index
	 *            - determines what bricks to assign different images to
	 */
	...
	public void renderLevelThree(SpriteBatch b, int index) {
		...
		sBatch.begin();
		if (index < 9) {
			sBatch.draw(brickImgs[0], brickShape.x, brickShape.y, getWidth(),
					getHeight());
		} else if (index >= 9 && index < 18) {
			sBatch.draw(brickImgs[1], brickShape.x, brickShape.y, getWidth(),
					getHeight());
		} else if (index >= 18 && index < 27) {
			sBatch.draw(brickImgs[2], brickShape.x, brickShape.y, getWidth(),
					getHeight());
		} else if (index >= 27 && index < 36) {
			sBatch.draw(brickImgs[3], brickShape.x, brickShape.y, getWidth(),
					getHeight());
		} else if (index >= 36 && index < 48) {
			sBatch.draw(brickImgs[4], brickShape.x, brickShape.y, getWidth(),
					getHeight());
		} else {
			sBatch.draw(brickImgs[5], brickShape.x, brickShape.y, getWidth(),
					getHeight());
		}
		sBatch.end();
	}
	 ...
	 *            - contains the rendering method
	 * @param index
	 *            - determine what bricks to assign different images to
	 */
