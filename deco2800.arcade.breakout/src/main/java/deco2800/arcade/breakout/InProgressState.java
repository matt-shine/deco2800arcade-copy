...
public class InProgressState extends GameState {
	...
	private void handlePowerupBallOtherCollision(GameScreen context) {
		if (overlap(context.getPowerupBall().getBallCirc(), context.getPaddle().getPaddleShape())
		...
		if (context.getPowerupBall().getY() >= context.SCREENHEIGHT - 2*context.getPowerupBall().getRadius()) {
		...
		if (context.getPowerupBall().getX() - context.getPowerupBall().getRadius() <= 0) {
			context.setLastHitX(context.getPowerupBall().getX());
			context.setLastHitY(context.getPowerupBall().getY());
			context.getPowerupBall().bounceX(5);
		}
		
		if (context.getPowerupBall().getX() + context.getPowerupBall().getRadius() > context.SCREENWIDTH) {
			context.setLastHitX(context.getPowerupBall().getX());
			context.setLastHitY(context.getPowerupBall().getY());
			context.getPowerupBall().bounceX(-5);
		}
	...
	private void handlePowerupBallBrickCollision(GameScreen context) {
		for (Brick b : context.getBrickArray()) {
			if (b.getState()) {
				if (b.checkLeftCollision(context.getPowerupBall().getBallCirc())) {
					b.setState(false);
				...
				if (b.checkRightCollision(context.getPowerupBall().getBallCirc())) {
					b.setState(false);
					if (Math.abs(context.getPowerupBall().getXVelocity()) < 80) {
						context.updateGameState(2, b, true);
						break;
				...
				if (b.checkTopCollision(context.getPowerupBall().getBallCirc())) {
					b.setState(false);
					context.updateGameState(1, b, true);
					break;
				}
				if (b.checkBottomCollision(context.getPowerupBall().getBallCirc())) {
					b.setState(false);
					context.updateGameState(1, b, true);
					break;
				}
	...
	private void handleOtherCollision(GameScreen context) {
		if (overlap(context.getBall().getBallCirc(), context.getPaddle().getPaddleShape())
		...
		if (context.getBall().getY() >= context.SCREENHEIGHT - 2*context.getBall().getRadius()) {
		...
		if (context.getBall().getX() + context.getBall().getRadius() > context.SCREENWIDTH) {
			context.setLastHitX(context.getBall().getX());
			context.setLastHitY(context.getBall().getY());
			context.getBall().bounceX(-5);
		}
	...
	public void handlePowerupCollision(GameScreen context) {
		context.getPowerupManager().checkCollision(context.getPaddle().getPaddleShape());
