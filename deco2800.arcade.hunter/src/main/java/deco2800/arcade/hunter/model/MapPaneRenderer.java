...
public class MapPaneRenderer {
	 ...
	 */
	public static TextureRegion renderPane(MapPane pane) {
		...
		for (int row = 0; row < Config.PANE_SIZE; row++) {
			for (int col = 0; col < Config.PANE_SIZE; col++) {
                int bgTileY = backgroundData[row][col] / tileMapWidthTiles * Config.TILE_SIZE;
                int bgTileX = (backgroundData[row][col] % tileMapWidthTiles) * Config.TILE_SIZE;

                int fgTileX = foregroundData[row][col] / tileMapWidthTiles * Config.TILE_SIZE;
                int fgTileY = (foregroundData[row][col] % tileMapWidthTiles) * Config.TILE_SIZE;

				tile.setColor(Color.CLEAR);
				tile.fill();
				//Draw the background tile image to the temporary pixmap
				tile.drawPixmap(tileMap, 0, 0, bgTileX, bgTileY, Config.TILE_SIZE, Config.TILE_SIZE);
				//Draw the foreground tile image to the temporary pixmap
				tile.drawPixmap(tileMap, 0, 0, fgTileX, fgTileY, Config.TILE_SIZE, Config.TILE_SIZE);
