...
public class LocalUserPaddle extends Paddle {
	...
	public void update(Ball ball) {
		...
		if (Gdx.input.isTouched()) {
			...
			double newY = ((currentHeight - touchPos.y) / currentHeight)
					* Pong.SCREENHEIGHT - bounds.height / 2;
			move(KBPADDLESPEED * Gdx.graphics.getDeltaTime()
					* sign(newY - prevY));
		...
		if (Gdx.input.isKeyPressed(Keys.UP)) {
			move(KBPADDLESPEED * Gdx.graphics.getDeltaTime());// leftPaddle.bounds.y
			// +=
			// KBPADDLESPEED
			// *
		...
		if (Gdx.input.isKeyPressed(Keys.DOWN)) {
			move(-KBPADDLESPEED * Gdx.graphics.getDeltaTime());// leftPaddle.position.y
			// -=
			// KBPADDLESPEED
			// *
			// Gdx.graphics.getDeltaTime();
