...
public class Enemy extends Mob {
	...
	/**
	 * Six possible states An enemy starts in either Stand or Path states.
	 * Enters chase state when player is spotted or gunfire is heard. Once enemy
	 * has left the stand or path states, it cannot reenter them.
	 */
	public enum STATES {
	...
	// when we reach the end of the path, instead of going back to 0, go back to
	// here
	...
	// suffers from pain (they have an animation that they do nothing in when
	// they get hit, interrupts their current action)
	...
	public void init(GameModel model) {
		...
		speed = 0.04f;// TODO remove this and let each enemy decide it's own
						// speed
	...
	public void tick(GameModel gameModel) {
		...
		if ((this.state == STATES.PATH || this.state == STATES.STAND)
				&& canSee(gameModel.getPlayer(), gameModel)) {
			this.setVel(new Vector2(0, 0));
	...
	public void initialiseFromEnemyData(DoodadInfo d) {
		...
		if (e == EnemyType.FETTGESICHT || e == EnemyType.GIFTMACHER
				|| e == EnemyType.GRETEL || e == EnemyType.HANS
				|| e == EnemyType.HITLER || e == EnemyType.SCHABBS) {
	...
	public void shootAtPlayer(GameModel g) {
		...
		int damage = calcDamage((int) p.getPos().dst(getPos()), this.getVel()
				.len() == 0, false);
		Projectile bullet = new Projectile(0, damage, true, "snowball");
		g.addDoodad(bullet);
	...
	protected void followPlayer(GameModel g) {
		...
		if (chaseGoal != null && chaseLast != null) {
			this.setVel(new Vector2().add(chaseGoal).sub(this.getPos()).nor()
					.mul(speed));
		} else {
			this.setVel(new Vector2(0, 0));
		}
	 ...
	 * but avoids tiles closer than (4.0 + random() * 1) units to the player
	...
	protected Vector2 bestChaseGoal(GameModel g) {
		...
		Vector2[] dirs = new Vector2[] { new Vector2(0, 1), new Vector2(1, 0),
		...
		for (int i = 0; i < dirs.length; i++) {
			...
			if (this.isWalkableTile(g, (int) Math.floor(hypothetical.x),
					(int) Math.floor(hypothetical.y))) {

				goals.put(
						(float) (hypothetical.dst(p.getPos()) + Math.random()),
						hypothetical.cpy());
	 ...
	 * @param speed
	 *            Is the player running or not
	 * @param look
	 *            Can the player see the shooter or not
	...
	public int calcDamage(int dist, boolean speed, boolean look) {
		...
		/*
		 * boolean hit = false; if (randInt(0, 255, getRand()) < ((speed ? 160 :
		 * 256) - (dist * (look ? 16 : 8)))) { hit = true; }
		 * 
		 * int damage = randInt(0, 255, getRand());
		 * 
		 * if (hit) { if (dist < 2) { damage = damage / 4; } else if (dist >= 2
		 * && dist < 4) { damage = damage / 8; } else if (dist >= 4) { damage =
		 * damage / 16; } else { damage = 0; } } else { damage = 0; }
		 * 
		 * return damage;
		 */
	}
		...
		while (!complete) {
			...
			if (gameModel.getWaypoint(x, y) != null) {
				if (usedWaypoints.contains(new Vector2(x, y))) {
					...
					angle = (int) WL6Meta.dirToAngle(gameModel
							.getWaypoint(x, y));
				}
			}
		}
